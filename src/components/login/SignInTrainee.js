...
export class SignInTrainee extends React.Component {
    state = { email: "", password: "", loading: false, recaptcha: null, data: [] }
    doLoginTrainee = async (event) => {
        event.preventDefault();
        await this.setState({ loading: true });
        try {
            const response = await validateTrainee(this.state.email, this.state.password);
            const data = await response.json();
            await this.props.changeUserSession(true);
            await this.props.tokenAction({ token: data.token })
            await this.props.authUser(data.traineeDetail);
                        ...
                        <Container component="main" maxWidth="xs" style={{ marginTop: "55px" }}>
                            ...
                            <div className={this.useStyles.paper}>
                                ...
                                <form className={this.useStyles.form} noValidate>
                                    ...
                                    <br></br>
                                    <div>
                                        <Button size="lg" className="btn btn-lg btn-block" onClick={this.doLoginTrainee} style={{ marginTop: "20px", backgroundColor: "#4287f5", border: "1px solid #4287f5" }}>
                                            Login
                                    </Button>
                                    </div>
...
export default connect(null, mapDispatchToProps)(SignInTrainee)